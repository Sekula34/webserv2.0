...
void ResponseBody::_handlerGetMethod()
{
    ...
    std::string path = _client.getClientMsg()->getRequestHeader()->urlSuffix->getPath();
...
void ResponseBody::_autoindexHtml(const std::string& serverFilePath, const LocationSettings& location)
{
    ...
    {
        _httpStatusCode = 0;
        Autoindex autoindex(serverFilePath, _httpStatusCode, _client.getClientMsg()->getRequestHeader()->getFullClientURL());
...
{
    std::string serverPath(_client.getClientMsg()->getRequestHeader()->urlSuffix->getPath());
...
{
    if(_client.getClientMsg()->getRequestHeader()->getRequestLine().protocolVersion != "HTTP/1.1")
    {
        _renderServerErrorPage(505);
    }
    std::string requstedMethod = _client.getClientMsg()->getRequestHeader()->getRequestLine().requestMethod;
